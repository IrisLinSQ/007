...
namespace WindowsFormsApplication1
{
    public partial class Cinema : Form
    {
        ...
        private void showTree_Click(object sender, EventArgs e)
        {
            ...
            String planDay = dateTimePicker2.Value.ToString("yyyy-MM-dd");

            DataSet ds1 = SqlPutPlan.getAllMoiveName(planDay);
            foreach (DataRow row1 in ds1.Tables[0].Rows)
            {
                ...
                DataSet ds2 = SqlPutPlan.getTimeByMoiveName(N1.Name,planDay);
       ...
       // 售票treeview 选择的内容改变
            ...
            if (this.tvMovies.SelectedNode.Level == 1)
            {
                ...
                foreach (DataRow row1 in ds1.Tables[0].Rows)
                {
                    ...
                    foreach (Control ct in tpCinema.Controls)
                    {
                        if (ct is Label)
                        {
                            if (ct.Text == ST.Trim()) { ct.BackColor = Color.Red; saleTicket++; }
                        }
                    }
                }
                noSaleTicket = screen.getCol() * screen.getRow() - saleTicket;
                this.saleTicket.Text = Convert.ToString(saleTicket);
                this.noSaleTicket.Text = Convert.ToString(noSaleTicket);
            }
        ...
        private void lb_Click(object sender, EventArgs e)
        {
            ...
            if (lbl.BackColor == Color.Red)
            {
                if (DialogResult.OK == MessageBox.Show("是否退票", "提示", MessageBoxButtons.OKCancel, MessageBoxIcon.Question))
                {
                    ...
                    this.saleTicket.Text = Convert.ToString(Convert.ToInt16(this.saleTicket.Text) - 1);
                    this.noSaleTicket.Text = Convert.ToString(Convert.ToInt16(this.noSaleTicket.Text) + 1);
                } 
            }
            else
            {
        ...
        private void button11_Click(object sender, EventArgs e)
        {
            ...
            DataSet ds1 = SqlPutPlan.getAllScreensName(planDay);
            foreach (DataRow row1 in ds1.Tables[0].Rows)
            {
                TreeNode N1 = new TreeNode();
                N1.Name = row1[0].ToString().Trim();
                ...
                DataSet ds2 = SqlPutPlan.getMoiveNameByScreenName(N1.Name.Trim(),planDay);
                foreach (DataRow row2 in ds2.Tables[0].Rows)
                {
                    TreeNode N2 = new TreeNode();
                    N2.Name = row2[0].ToString().Trim();
                    ...
                    DataSet ds3 = SqlPutPlan.getTimeByMoiveScressName(N2.Name.Trim(), N1.Name.Trim(),planDay);
                    foreach (DataRow row3 in ds3.Tables["time"].Rows)
                    {
                        TreeNode N3 = new TreeNode();

                        //int length = SqlMoive.getLengthByMoive(N2.Text);
                        //int l = SqlPutPlan.changeToMinute(row3[0].ToString().Trim());
                        //length += l;
                        //string t = SqlPutPlan.changeTime(length);
        ...
        private void button9_Click(object sender, EventArgs e)
        {
            if (comboBox3.SelectedItem != null)
            {
                ...
                int allStuNumber, todayStuNumber;
                int allNormalNumber, todayNormalNumber;
                int allFreeNumber, todayFreeNumber;
                float allMoneyNumber, todayMoneyNumber;
                float allSaleMoneyNumber, todaySallMoneyNumber;

                allnumber = SqlMoive.allMoiveNumber(moive);
                todaynumber = SqlMoive.singleMoiveNumber(moive);
                allNormalNumber = SqlMoive.allNormalNumber(moive);
                todayNormalNumber = SqlMoive.todayNormalNumbee(moive);
                allFreeNumber = SqlMoive.allFreeNumber(moive);
                todayFreeNumber = SqlMoive.todayFreeNumber(moive);
                allFreeNumber = SqlMoive.allFreeNumber(moive);
                todayFreeNumber = SqlMoive.todayFreeNumber(moive);
                todayStuNumber = SqlMoive.todayStuNumber(moive);
                allStuNumber = SqlMoive.allStuNumber(moive);
                allMoneyNumber = SqlMoive.allMoneyNumber(moive);
                todayMoneyNumber = SqlMoive.todayMoneyNumber(moive);
                allSaleMoneyNumber = SqlMoive.allSaleMoneyNumber(moive);
                todaySallMoneyNumber = SqlMoive.todaySaleMoneyNumber(moive);                

                textBox6.Text = Convert.ToString(allnumber);
                textBox3.Text = Convert.ToString(todaynumber);
                textBox2.Text = Convert.ToString(allNormalNumber);
                textBox4.Text = Convert.ToString(todayNormalNumber);
                textBox7.Text = Convert.ToString(allFreeNumber);
                textBox9.Text = Convert.ToString(todayFreeNumber);
                textBox12.Text = Convert.ToString(todayStuNumber);
                textBox10.Text = Convert.ToString(allStuNumber);
                textBox13.Text = Convert.ToString(allMoneyNumber);
                textBox14.Text = Convert.ToString(todayMoneyNumber);
                textBox15.Text = Convert.ToString(string.Format("{0:C}", allSaleMoneyNumber));
                textBox16.Text = Convert.ToString(string.Format("{0:C}", todaySallMoneyNumber));
            }
            else
            {
        ...
        private void button10_Click(object sender, EventArgs e)
        {
            ...
            {
                int screen = Convert.ToInt16(comboBox4.SelectedItem.ToString());
                int allnumber, todaynumber;
                int allStuNumber, todayStuNumber;
                int allNormalNumber, todayNormalNumber;
                int allFreeNumber, todayFreeNumber;
                ...
                allnumber = SqlScreens.allScreenNumber(screen);
                todaynumber = SqlScreens.singleScreenNumber(screen);
                allNormalNumber = SqlMoive.allNormalNumber(screen);
                todayNormalNumber = SqlMoive.todayNormalNumbee(screen);
                allFreeNumber = SqlMoive.allFreeNumber(screen);
                todayFreeNumber = SqlMoive.todayFreeNumber(screen);
                allFreeNumber = SqlMoive.allFreeNumber(screen);
                todayFreeNumber = SqlMoive.todayFreeNumber(screen);
                todayStuNumber = SqlMoive.todayStuNumber(screen);
                allStuNumber = SqlMoive.allStuNumber(screen);
                //allMoneyNumber = SqlMoive.allMoneyNumber(screen);
                //todayMoneyNumber = SqlMoive.todayMoneyNumber(screen);
                allSaleMoneyNumber = SqlMoive.allSaleMoneyNumber(screen);
                todaySallMoneyNumber = SqlMoive.todaySaleMoneyNumber(screen);
               ...
               //textBox13.Text = Convert.ToString(allMoneyNumber);
               //textBox14.Text = Convert.ToString(todayMoneyNumber);
                textBox15.Text = Convert.ToString(allSaleMoneyNumber);
                textBox16.Text = Convert.ToString(todaySallMoneyNumber);
            }
            else
            {
                MessageBox.Show("请选择你要查询的电影");
